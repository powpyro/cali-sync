...
function initialiserBaseDeDonnees(ss) {
  ...
  if (!sheetEval) {
    ...
    sheetEval.appendRow(["identifiant", "nom_complet", "date_creation", "nombre_sessions"]);
    sheetEval.appendRow(["oumar.toure", "Oumar Touré", new Date().toISOString(), 1]);
    sheetEval.appendRow(["jean.dupont", "Jean Dupont", new Date().toISOString(), 1]);
  }
...
function traiterRequete(e) {
  ...
  if (action === "creer_evaluateur") return jsonResponse(handleCreerEvaluateur(ss, req.identifiant, req.nom_complet || req.nomComplet));
...
function handleLogin(ss, identifiant) {
  ...
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]).trim().toLowerCase() === String(identifiant).trim().toLowerCase()) {
      return { success: true, exists: true, profil: { identifiant: data[i][0], nom_complet: data[i][1], date_creation: data[i][2], nombre_sessions: data[i][3] || 0 } };
    }
  }
...
function handleListerEvaluateurs(ss) {
  ...
  const list = [];
  for (let i = 1; i < data.length; i++) {
    list.push({ identifiant: data[i][0], nom_complet: data[i][1], date_creation: data[i][2], nombre_sessions: data[i][3] || 0 });
  }
  return { success: true, evaluateurs: list };
}

function handleCreerEvaluateur(ss, identifiant, nomComplet) {
  ...
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]).trim().toLowerCase() === String(identifiant).trim().toLowerCase()) return { success: true, message: "Évaluateur existant." };
  }
  sheet.appendRow([identifiant.trim().toLowerCase(), nomComplet.trim(), new Date().toISOString(), 0]);
  return { success: true, message: "Évaluateur créé avec succès." };
}
...
function handleGetCockpit(ss, sessionId) {
  ...
  for (var k = 1; k < subData.length; k++) {
    ...
    var rIsGauge = subData[k][3] === true || String(subData[k][3]).toUpperCase() === "TRUE";
